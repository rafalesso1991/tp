        ...
        try (Scanner scanner = new Scanner(file)) {
            while (scanner.hasNextLine()) {
                ...
                if (parts.length == 8) {
                    ...
                    int vida = Integer.parseInt(parts[8]);

                    // POLIMORFISMO
                    // Instancia la subclase de personaje según el rol elegido.
                    switch (rol) {
                        case GUERRERO:
                            personajesCargados.add(new Guerrero(nombre, raza, fuerza, agilidad, inteligencia, voluntad, carisma, vida));
                            break;
                        case MAGO:
                            personajesCargados.add(new Mago(nombre, raza, fuerza, agilidad, inteligencia, voluntad, carisma, vida));
                            break;
                        case LADRON:
                            personajesCargados.add(new Ladron(nombre, raza, fuerza, agilidad, inteligencia, voluntad, carisma, vida));
                            break;
                        case SACERDOTE:
                            personajesCargados.add(new Sacerdote(nombre, raza, fuerza, agilidad, inteligencia, voluntad, carisma, vida));
                            break;
                        case BARDO:
                            personajesCargados.add(new Bardo(nombre, raza, fuerza, agilidad, inteligencia, voluntad, carisma, vida));
                            break;
                    }
                }
            }
